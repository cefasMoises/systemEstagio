    ...
    * Your data must be a json string (not object) with the keys <value> and <label>
    ...
    .display-area {
        ...
        scroll-behavior: smooth;
                    ...
                    <div class="relative bw-select bw-select-{{$input_name}} @if($add_clearing) mb-3 @endif" role="combobox" data-multiple="{{$multiple}}" data-required="{{$required?'true':'false'}}" data-type="{{ $data !== 'manual' ? 'dynamic' : 'manual'}}" @if(!empty($filter)) data-filter="{{ $filter}}" @endif @if(!empty($meta)) data-meta-data="{{ $meta}}" @endif @if($data=='manual' && $selected_value !='' ) data-selected-value="{{implode(',',$selected_value)}}" @endif>
                        <div tabindex="0" class="flex justify-between text-sm items-center rounded-md bg-white text-slate-600 border-2 border-slate-300/50 hover:border-slate-300
         ...
         @if($disabled) disabled @elseif($readonly) readonly @else enabled @endif">
                            ...
                            <div class="whitespace-nowrap inline-flex">
                                ...
                                <x-bladewind::icon name="x-circle" type="solid" class="hidden reset size-6 text-white fill-gray-400/70 hover:fill-gray-400 dark:fill-white/40 dark:hover:fill-white/60" />
        ...
        hidden bw-select-items-container overflow-scroll max-h-64 animate__animated animate__fadeIn animate__faster">
                            <div class="sticky top-0 min-w-full bg-slate-100 dark:bg-transparent py-1 pr-0 -pl-1 @if(!$searchable) hidden @endif">
                                <x-bladewind::input class="!border-0 !border-b !rounded-none focus:!border-slate-300 dark:focus:!border-slate-600 !w-full !text-sm bw_search" add_clearing="false" :placeholder="$search_placeholder" suffix="magnifying-glass" suffixIsIcon="true" />
                            </div>
                            <div class="divide-y divide-slate-100 dark:divide-slate-600/70 bw-select-items mt-0">
                                @if($data !== 'manual')
                                @forelse ($data as $item)
                                <x-bladewind::select-item label="{{ $item[$label_key] }}" value="{{ $item[$value_key] }}" filter_by="{{ ($filter_by != '') ? $item[$filter_by] : '' }}" onselect="{{ $onselect }}" flag="{{ $item[$flag_key] ?? '' }}" image="{{ $item[$image_key] ?? '' }}" selected="{{ (in_array($item[$value_key], $selected_value)) ? 'true' : 'false' }}" />
                                @empty
                                @if($empty_state)
                                <x-bladewind::select-item :selectable="false" :empty_state="true" :empty_state_message="$empty_state_message" :empty_state_show_image="$empty_state_show_image" :empty_state_button_label="$empty_state_button_label" empty_state_onclick="{!! $empty_state_onclick !!}" :empty_state_image="$empty_state_image" />
                                @else
                                <x-bladewind::select-item :selectable="false" :label="$empty_placeholder" />
                                @endif
                                @endforelse
                                @else
                                {!! $slot !!}
                        ...
                        <input type="hidden" name="{{ ($data_serialize_as !== '') ? $data_serialize_as : $input_name }}" class="bw-{{$input_name}} @if($required) required @endif" @if($required) data-parent="bw-select-{{$input_name}}" @endif @if($multiple) autocomplete="off" @endif />
                    </div>
                    <script>
